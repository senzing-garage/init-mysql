...
"""
# -----------------------------------------------------------------------------
# init-mysql initializes a MySQL database for use with Senzing.
#   - Creates the schema (tables, indexes, etc.)
#   - Inserts initial Senzing configuration
#   - init-mysql.py is idempotent.  It can be run repeatedly.
# -----------------------------------------------------------------------------
"""
...
CONFIGURATION_LOCATOR = {
    ...
    "input_sql_url": {
        "default": "/opt/senzing/g2/resources/schema/g2core-schema-mysql-create.sql",
...
def get_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="init-mysql.py",
        description="Add description. For more information, see https://github.com/senzing-garage/init-mysql",
    )
...
def create_senzing_database_connection_string(database_url):
    ...
    return "{scheme}://{username}:{password}@{hostname}:{port}/?schema={schema}".format(
        **parsed_database_url
    )
...
def process_sql_file(input_url, db_parameters):
    ...
    db_connection = mysql.connector.connect(**db_parameters)
                    ...
                    try:
                        ...
                        logging.error(
                            message_error(
                                702,
                                "{0} - {1}".format(
                                    type(error), " ".join(str(error).split())
                                ),
                            )
                        )
...
def create_database_url(senzing_database_connection_string):
    """Replace the last instance of a character."""

    parsed_database_url = parse_database_url(senzing_database_connection_string)
    parsed_query_string = parse_qs(parsed_database_url.get("query", ""))
    parsed_database_url["schema"] = parsed_query_string.get("schema")[0]
...
def task_process_sql_file(config):
    ...
    if engine_configuration_json:
        ...
        if db_url_raw:
            db_parameters_list.append(create_database_url(db_url_raw))
        ...
        if cluster_key:
            if cluster_key == "SQL":
                ...
                for cluster_value in cluster_values_set:
                    cluster_db_raw = engine_configuration.get(cluster_value, {}).get(
                        "DB_1"
                    )
                    db_parameters_list.append(create_database_url(cluster_db_raw))
